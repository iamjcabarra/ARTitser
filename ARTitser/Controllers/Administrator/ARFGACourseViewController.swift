//
//  ARFGACourseViewController.swift
//  ARFollow
//
...
class ARFGACourseViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, NSFetchedResultsControllerDelegate, UISearchBarDelegate, ARFGACourseCreationViewControllerDelegate {
        ...
        self.arfDataManager.requestRetrieveCourses() { (result) in
            ...
            if status == 0 {
                DispatchQueue.main.async {
                    ...
                    self.courseCount = count
                    self.title = "Courses (\(count))"
    ...
    fileprivate func customizeNavigationBar() {
        ...
        addButton.setImage(ARFConstants.image.GAV_ADD_COURSE, for: UIControlState())
    ...
    /// Presents course details view if deep copying of
    /// course object succeeded where user can create a
    /// new course.
    ///
    /// - parameter sender: A UIButton
    @objc fileprivate func addButtonAction(_ sender: UIButton) {
        self.arfDataManager.deepCopyCourseObject(nil, owner: self.loggedUserId, isCreation: true) { (result) in
            if result != nil {
                DispatchQueue.main.async {
                    let course = result!["course"] as! DeepCopyCourse
                    let data: [String: Any] = ["course": course, "isCreation": true]
                    self.performSegue(withIdentifier: ARFConstants.segueIdentifier.GAV_COURSE_CREATION_VIEW, sender: data)
                }
            }
            else {
                DispatchQueue.main.async {
    ...
    fileprivate func shouldShowEmptyPlaceholderView(_ show: Bool) {
        var shouldShow = show
        if (self.searchBar.text == "") { shouldShow = self.courseCount > 0 ? false : true }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: ARFConstants.cellIdentifier.COURSE, for: indexPath) as! ARFGACourseTableViewCell
        self.configureCell(cell, atIndexPath: indexPath)
        return cell
    }
    
    func configureCell(_ cell: ARFGACourseTableViewCell, atIndexPath indexPath: IndexPath) {
        ...
        cell.courseDescriptionLabel.text = courseObject.courseDescription!
    }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        let courseObject = fetchedResultsController.object(at: indexPath) as! Course
        let data: [String: Any] = ["course": courseObject]
        self.performSegue(withIdentifier: ARFConstants.segueIdentifier.GAV_COURSE_DETAILS_VIEW, sender: data)
    }
    ...
    func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        ...
        let updateAction = UIContextualAction(style: .normal, title: "", handler: { (action: UIContextualAction, view: UIView, success: (Bool) -> Void) in
            self.arfDataManager.deepCopyCourseObject(courseObject, owner: self.loggedUserId, isCreation: false) { (result) in
                if result != nil {
                    DispatchQueue.main.async {
                        let course = result!["course"] as! DeepCopyCourse
                        let data: [String: Any] = ["course": course, "isCreation": false]
                        self.performSegue(withIdentifier: ARFConstants.segueIdentifier.GAV_COURSE_CREATION_VIEW, sender: data)
                    }
                }
                else {
        ...
        let deleteAction = UIContextualAction(style: .normal, title: "", handler: { (action: UIContextualAction, view: UIView, success: (Bool) -> Void) in
            let message = "Are you sure you want to delete \"\(courseObject.code!): \(courseObject.title!)\"?"
            let alert = UIAlertController(title: "Delete Course", message: message, preferredStyle: .alert)
            
            let posAction = UIAlertAction(title: "Yes", style: .default) { (Alert) -> Void in
                HUD.show(.rotatingImage(ARFConstants.image.GEN_PROGRESS))
                
                self.arfDataManager.requestDeleteCourse(withId: "\(courseObject.id)", completion: { (result) in
                    let status = result!["status"] as! Int
                    ...
                    if status == 0 {
                        DispatchQueue.main.async {
                            HUD.hide()
                            let subtitle = result!["message"] as! String
                            HUD.flash(.labeledSuccess(title: "Delete Course", subtitle: subtitle), onView: nil, delay: 3.5, completion: { (success) in
                                self.reloadCourseList()
                        ...
                        DispatchQueue.main.async {
                            HUD.hide()
                            let subtitle = result!["message"] as! String
                            HUD.flash(.labeledError(title: "Delete Course", subtitle: subtitle), onView: nil, delay: 3.5, completion: { (success) in
                                alert.dismiss(animated: true, completion: nil)
                            })
                        }
                    }
        ...
        case .update:
            if let indexPath = indexPath {
                if let cell = self.tableView.cellForRow(at: indexPath) {
                    self.configureCell(cell as! ARFGACourseTableViewCell, atIndexPath: indexPath)
                }
            }
            break;
        case .move:
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if segue.identifier == ARFConstants.segueIdentifier.GAV_COURSE_DETAILS_VIEW {
            guard let data = sender as? [String: Any], let course = data["course"] as? Course else {
                print("ERROR: Can't parse segue sender's data!")
                return
            }
            
            let courseDetailsView = segue.destination as! ARFGACourseDetailsViewController
            courseDetailsView.course = course
        }
        
        if segue.identifier == ARFConstants.segueIdentifier.GAV_COURSE_CREATION_VIEW {
            guard let data = sender as? [String: Any], let course = data["course"] as? DeepCopyCourse, let isCreation = data["isCreation"] as? Bool else {
                print("ERROR: Can't parse segue sender's data!")
                return
            }
            
            let courseCreationView = segue.destination as! ARFGACourseCreationViewController
            courseCreationView.course = course
            courseCreationView.isCreation = isCreation
            courseCreationView.delegate = self
        }
        
    }
    
    // MARK: - ARFGACourseCreationViewControllerDelegate
